...
public class TomVM extends HasErrorState implements Streamable {
	...
	// External functions
	/**
	 * functions are standard functions where the parameters are pushed to the
	 * stack.
	 */
	 ...
	 * <p>
	 * (When evaluating parameters for a impending function call,
	 * the pending function stack frame is the top of the stack, but is not yet
	 * active).
	 */
	 ...
	 * <p>
	 * (e.g declared with "DATA" keyword in BASIC)
	 */
	...
	/**
	 * Set to true if breakpoints are patched and in synchronisation with compiled
	 * code
	 */
	...
	public void continueVM(int steps) // Continue execution from last position
	{
		...
		step: while (true) { // breaks on last line; taking advantage of loops having labels for continue
			...
			switch (instruction.opCode) {
				...
				case OpCode.OP_ARRAY_INDEX:
					if (getReg2().getIntVal() != 0) {
						...
						if (getReg().getIntVal() >= 0
								&& getReg().getIntVal() < data.data().get(getReg2().getIntVal()).getIntVal()) {
							assertTrue(
				...
				case OpCode.OP_CHECK_FUNC_PTR: {
					...
					// Check function prototype is compatible with prototype referenced by
					// instruction
					UserFuncPrototype srcProto = userFunctionPrototypes
							.get(userFunctions.get(funcIndex).prototypeIndex);
				...
				case OpCode.OP_SAVE_PARAM_PTR: {
					...
					assertTrue(!userCallStack.isEmpty());
					userCallStack
							.lastElement().localVarDataOffsets
	...
	void copyArray(int sourceIndex, int destIndex, ValType type) {
		...
		assertTrue(data.isIndexValid(sourceIndex));
		assertTrue(data.isIndexValid(destIndex));
		assertTrue(data.data().get(sourceIndex).getIntVal() == data.data().get(destIndex).getIntVal()); // Array sizes
																										// match
		assertTrue(data.data().get(sourceIndex + 1).getIntVal() == data.data().get(destIndex + 1).getIntVal()); // Element
																												// sizes
																												// match
	...
	public void addStepBreakPoints(boolean stepInto) {
		...
		for (int i = startOffset; i < endOffset; i++) {
			// TODO had to reduce dest from 0xffffffff since Java does not like unsigned
			// values
			int dest = 0x7fffffff;
	 ...
	 * then application can block and wait for next event before calling continue()
	 * again.
	 * 
	 * @return
	 */
