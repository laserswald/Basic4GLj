...
public class TomBasicCompiler extends HasErrorState {
	...
	// TODO Replace all Vector with ArrayList
	// TODO Use interfaces for member variables
	...
	// Parser
	private final Tokenizer parser;
	...
	private final List<Library> libraries = new Vector<>();
	...
	private final List<ValType> operandStack;
	private final List<StackedOperator> operatorStack;
	 ...
	 * Resets to fix up
	 */
	private final List<Jump> resets;
	/**
	...
	/**
	 * True if need to generate code to free temporary data before the next
	 * instruction
	 */
	...
	/**
	 * Combines local and global (where a local function overrides a global one of
	 * the same name)
	 */
	 ...
	 * can be different in special cases (e.g. when compiler is called from debugger
	...
	private final List<com.basic4gl.compiler.RuntimeFunction> runtimeFunctions;
	 ...
	 */
	private final List<UnaryOperatorExtension> unaryOperatorExtensions;
	/**
	 * Binary operator extensions
	 */
	private final List<BinaryOperatorExtension> binaryOperatorExtensions;
	 ...
	 */
	static record Operator(OperType type, short opCode, int params, int binding) {
		Operator() {
			this(OperType.OT_OPERATOR, OpCode.OP_NOP, 0, 0);
		}
	}

	static record StackedOperator(Operator operator, int lazyJumpAddress) {
	}
	 ...
	 * jumps are possible.)
	 */
	record Jump(int jumpInstruction, String labelName) {
	}
	...
	public enum LanguageSyntax {
		...
		/**
		 * Like LS_TRADITIONAL, but also tries to match the variable suffixes of other
		 * BASIC types.
		 */
	...
	public TomBasicCompiler(TomVM vm, PluginManager plugins, boolean caseSensitive) {
		...
		reservedWords.add("arraymax"); // Library mechanism doesn't support any-type parameters. Compiler needs to
										// treat arraymax() as a special case
	...
	UserFuncPrototype getCurrentUserFunctionPrototype() {
		...
		assertTrue(getCurrentUserFunction().prototypeIndex < vm.getUserFunctionPrototypes().size());
	...
	private boolean compileBeginCodeBlock() {
		...
		// This affects "runtime" function definitions and "GetFunctionByName"
		// behaviour.
		// Programs can lookup code blocks at runtime with "GetCodeBlockByName"
		// function.

		// Note for Note: "I" refers to Tom, the original author of this code :)
		// Note: This was implemented mainly so I could port my Thunderbolt game to
		// Basic4GL Mobile,
		// without having to include the whole compiler in the runtime.
		// I don't expect this command will benefit anyone else, and would probably be
		// more
		...
		addInstruction(OpCode.OP_JUMP, BasicValType.VTP_INT, new Value(0)); // Offset will be filled in when code block
																			// ends
	...
	private boolean compileStructure() {
		...
		if (vm.getDataTypes().isStrucStored(name)) {
			...
			// If already declared we can define it, but only if it has not already been
			// defined
	...
	private boolean compileDim(boolean forStruc, boolean forFuncParam, Mutable<UserFuncPrototype> prototype) {
		...
		while (!atSeparatorOrSpecial() && (!forFuncParam || !token.getText().equals(")"))) {
			...
			if (forStruc) {
				...
				// Embedded structures must be fully defined (pointers to declared but undefined
				// structures are okay though)
				...
				if (inFunction) {
					...
					if (varIndex >= 0) {
						if (!(getCurrentUserFunctionPrototype().localVarTypes
								.get(varIndex)
								.equals(type))) {
	...
	private boolean compileOperation() {
		...
		if (o.operator.params == 1) {
			...
			if (o.operator.type == OperType.OT_OPERATOR
					&& compileOverloadedOperator(o.operator.opCode(), FunctionType.FT_OPERATOR_UNARY)) {
				return true;
			}
			...
			// Try overloaded operator functions first
			if (o.operator.type == OperType.OT_OPERATOR
					&& compileOverloadedOperator(o.operator.opCode(), FunctionType.FT_OPERATOR_BINARY)) {
				return true;
			}
			// NOTE: commented out in newer versions of the original compiler code
			// Try plug in language extension first
			...
			if (regType.isNull() || reg2Type.isNull()) {
				...
				// Can compare function pointers to other function pointers with compatible
				// prototypes.
				// Operator must be '=' or '<>'.
				if (o.operator.opCode != OpCode.OP_OP_EQUAL && o.operator.opCode != OpCode.OP_OP_NOT_EQUAL) {
					setError("Operator cannot be applied to this data type");
	...
	private boolean compileOverloadedOperator(int opCode, FunctionType type) {
		...
		// NOTE: added verbose = false parameter to compileFunctionCall to prevent noisy
		// error messages being logged
		// if overload function is not found since this is a normal situation (i.e. most
		// operators won't be overloaded).
	...
	private boolean compileLoadConst() {
		...
		if (token.getTokenType() == TokenType.CTT_CONSTANT) {
			...
			if (token.getValType() == BasicValType.VTP_STRING) {
				...
				String text;
				// TODO - 2026-04-23 Need to review this code. original source used
				// `m_text.substr (1, m_token.m_text.length () - 1)`
	...
	private boolean compileConvert(int basictype) {
		...
		// Can convert function pointer, untyped function pointer or null to a function
		// pointer.
		// Note: Calling code is responsible for ensuring functions have compatible
		// prototypes
	...
	private boolean compileConvert(ValType type) {
		...
		if (type.isBasicType()) {
			...
			// Note: Exact equals is required as == will say that pointers are equal to
			// references.
			// (Internally this is true, but we want to enforce that programs use the
			// correct type.)
			return true;
		}
		...
		// Copy object case
			...
			if (regType.arrayLevel == reg2Type.arrayLevel && regType.basicType == reg2Type.basicType) {
				...
				if (vm.getDataTypes().containsPointer(dataType)) {
					addInstruction(OpCode.OP_CHECK_PTRS, BasicValType.VTP_INT,
	...
	private boolean compileFunctionCall(String name, FunctionType type, boolean needResult, boolean verbose) {
		...
		// (We need to decide up front whether a '(' is the function brackets or part of
		// the first parameter expression.
		// It gets too complicated if we try to handle some overloads that require
		// brackets and some that don't.)
		boolean brackets = Arrays.stream(functions).anyMatch(fn -> {
			...
			if ((syntax == LS_TRADITIONAL || syntax == LS_TRADITIONAL_SUFFIX) && (!fn.getSpecification().isFunction()
		...
		boolean isNoParamFunction = functionCount == 1 && functions[0].getSpecification().isFunction()
				&& functions[0].getSpecification().getParamTypes().getParams().size() == 0;
		...
		while (functionCount > 0 &&
				((type == FunctionType.FT_NORMAL && !token.getText().equals(")") && !atSeparatorOrSpecial()
						...
						|| (type == FunctionType.FT_OPERATOR_BINARY && count < 2))) { // Exit criteria for binary
																						// functions
	...
	private boolean compileArrayMax() {
		...
		// Function takes a parameter and returns a value. Thus it requires brackets in
	...
	boolean compileUserFunctionAddress() {
		...
		// (This makes the code simpler, as we don't have to have any special
	...
	ExtendedFunctionSpecification findExtendedFunction(String name, boolean hasParam, int paramBasicValType) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ((params.isEmpty() && !hasParam) ||
					(params.size() == 1 && hasParam && (paramBasicValType == BasicValType.VTP_UNDEFINED
	 ...
	 * 
	 * @param funcType
	 * @param funcPointerTypeRef used to return corresponding function pointer type,
	 *                           if funcType = UFT_POINTER
	 * @return
	 */
	private boolean compileUserFunction(UserFunctionType funcType, Mutable<ValType> funcPointerTypeRef) {

		// Note: funcPointerType is used to return corresponding function pointer type,
		// if funcType = UFT_POINTER
	...
	private boolean compileUserFuncParam(Mutable<UserFuncPrototype> prototype, int i) {
		...
		if (type.isBasicType()) {
			...
			// Attempt to convert value in reg to same type
			// Note: Use full type, so that prototype # is taken into account for function
			// pointer types.
