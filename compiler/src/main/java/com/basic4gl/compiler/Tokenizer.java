...
public class Tokenizer extends HasErrorState {

	// The input text, broken into lines.
	...
	// The location in the input text.
	private int line;
	private int column;
	...
	// Read a character from the input string, updating the location as we go.
	char getChar(boolean inString) {
		char c = peekChar(inString);
	...
	char peekChar(boolean inString) {
		...
		} else if (column >= getText().length() || (getText().charAt(column) == '\'' && !inString)) // End of line,
		// or start of
		// comment
		{
			return 13;
	...
	public Token nextToken(boolean skipEOL, boolean dataMode) {
		...
		if (c == 0) {
			...
			if (isNumber((char) c)) {
				...
				t.setTokenType(TokenType.CTT_IDENTIFIER);
			} else {
		...
		if (!dataMode || t.getTokenType() != TokenType.CTT_IDENTIFIER) {
				...
				switch (t.getTokenType()) {
					case CTT_CONSTANT:
						if (t.getValType() == BasicValType.VTP_STRING) {
							...
							boolean hexSpecifier = lcase == 'x'
									&& (t.getText().equals("0") || t.getText().equals("-0"));
							if (hexSpecifier) {
								hex = true;
							}
							done = !((c >= '0' && c <= '9')
